...
const PORT_WAIT_TIMEOUT = PORT_WAIT_TIMEOUT_STR
  ...
  : 120_000; // 2 minutes for Node.js workers (FastStore/Next.js can take longer to start)

export interface NodeIsolateOptions extends IsolateOptions {
  packageManager?: "npm" | "yarn" | "pnpm";
  script?: string; // npm script to run (default: "dev")
}
...
const isCmdIsolate = (
  cmd: NodeIsolateOptions | CommandIsolate,
): cmd is CommandIsolate => {
  return (cmd as CommandIsolate).command !== undefined;
};

export class NodeRun implements Isolate {
  ...
  constructor(options: NodeIsolateOptions | CommandIsolate) {
    if (isCmdIsolate(options)) {
      ...
      this.command = new Deno.Command(packageManager, {
        args: ["run", script],
        cwd: options.cwd,
        stdout: "piped",
        stderr: "piped",
