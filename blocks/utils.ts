...
async (
  ...
  return {
    ...
    inputSchema: fn.params.length > 0 && fn.params[0]
      ? await tsTypeToSchemeable(transformationContext, fn.params[0], root)
      : undefined,
  };
};
...
async (
  ...
  return {
    ...
    inputSchema: configType
      ? await tsTypeToSchemeable(transformationContext, configType, root)
      : undefined,
    outputSchema: fn.return
      ? await tsTypeToSchemeable(transformationContext, fn.return, root)
      : undefined,
  };
};
  ...
  async (ctx: TransformContext, path: string, ast: ASTNode[]) => {
    ...
    return {
      ...
      inputSchema: inputTsType
        ? await tsTypeToSchemeable(ctx, inputTsType, root)
        : undefined,
    };
  };
