...
import { applyProps, FnContext, FnProps, RequestState, SingleFlightKeyFunc } from './utils.tsx'
...
export interface LoaderModule<TProps = any, TState = any> extends BlockModule<FnProps<TProps>> {
    cache?: 'no-store' | 'stale-while-revalidate'
    // a null value avoid cache
    cacheKey?: (props: TProps, req: Request, ctx: FnContext<TState>) => string | null
...
export const isWrappedError = (err: any | WrappedError): err is WrappedError => {
...
export const wrapCaughtErrors = async <
    TConfig = any,
    TContext extends ResolverMiddlewareContext<any> = ResolverMiddlewareContext<any>,
>(
    _props: TConfig,
    ctx: TContext,
) => {
    if (isInvokeCtx(ctx)) {
        // invoke should not wrap caught errors.
        return ctx.next!()
    }
    try {
 ...
 */
const wrapLoader = (
    { default: handler, cache: mode = 'no-store', cacheKey = noop, singleFlightKey, ...rest }: LoaderModule,
    ...
    if (typeof singleFlightKey === 'function') {
        console.warn('singleFlightKey is deprecated and does not work anymore. Please use cacheKey instead')
    }
    ...
    return {
        ...
        default: async (
            ...
            try {
                // Should skip cache
                if (mode === 'no-store' || !ENABLE_LOADER_CACHE || !isCache(maybeCache) || cacheKeyValue === null) {
                    status = 'bypass'
                ...
                const resolveChainString = FieldResolver.minify(resolveChain).toString()
                ...
                if (resolveChainString && revisionID) {
                    ...
                    if (!revisionID) {
                        logger.warning(`Could not get revisionID for resolveChain ${resolveChainString}`)
                    }
                    if (!resolveChainString) {
                        logger.warning(`Could not get resolveChain for revisionID ${revisionID}`)
                    }
...
const loaderBlock: Block<LoaderModule> = {
    ...
    adapt: <TProps = any>(mod: LoaderModule<TProps>) => [
        ...
        (props: TProps, ctx: HttpContext<{ global: any } & RequestState>) =>
            applyProps(wrapLoader(mod, ctx.resolveChain, ctx.context.state.release))(props, ctx),
    ],
