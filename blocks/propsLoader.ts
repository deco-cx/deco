...
import { Diff, Intersection, OptionalKeys, Overwrite, RequiredKeys } from '../deps.ts'
import { Promisified, waitKeys } from '../engine/core/utils.ts'
import { FnContext } from '../types.ts'

export type PropsUnion<TLoadProps, TSectionInput> = TSectionInput extends Record<string, any>
    ? Overwrite<
          TSectionInput,
          Overwrite<
              {
                  [key in keyof Diff<Required<TSectionInput>, Pick<TLoadProps, RequiredKeys<TLoadProps>>>]: Diff<
              ...
              }, // non-required keys in the input object that are required in the section object are required.
              {
                  [key in keyof Intersection<
                      Pick<TLoadProps, RequiredKeys<TLoadProps>>,
                      Required<TSectionInput>
                  >]?: Intersection<Pick<TLoadProps, RequiredKeys<TLoadProps>>, Required<TSectionInput>>[key]
              } & {
                  // required keys in both are optional in the loader
                  [key in keyof Pick<TSectionInput, OptionalKeys<TSectionInput>>]?: Pick<
                      TSectionInput,
...
export type ObjectLoader<TLoaderProps, TSectionInput> = {
    [key in keyof PropsUnion<TLoaderProps, TSectionInput>]:
        | PropsUnion<TLoaderProps, TSectionInput>[key]
        | FnProps<TLoaderProps, PropsUnion<TLoaderProps, TSectionInput>[key]>
}
...
const isLoaderFunc = <TLoaderProps, TSectionInput>(
    obj: ObjectLoader<TLoaderProps, TSectionInput> | TSectionInput | FnProps<TLoaderProps, TSectionInput>,
...
export const propsLoader = async <TSectionInput, TProps>(
    ...
    for (const key of Object.keys(resolver)) {
        ...
        if (isLoaderFunc(funcOrValue)) {
            ...
            resolved[keyAsResolvedKey] = result as (typeof resolved)[typeof keyAsResolvedKey]
        } else {
