...
export const generateEditorData = async <Data = unknown>(
  ...
  const schemas = (context.manifest?.definitions ?? {}) as Record<
    string,
  ...
  const sectionsWithSchema = sections.map(
    (section): EditorData["sections"][0] => ({
      ...section,
      schema: schemas[section.key]?.inputSchema || undefined,
    }),
  );
  ...
  const functionsWithSchema = functions.map(
    (functionData): EditorData["functions"][0] => ({
      ...functionData,
      schema: schemas[functionData.key]?.inputSchema || undefined,
      outputSchema: schemas[functionData.key]?.outputSchema || undefined,
    }),
  );
