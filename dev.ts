...
async function extractComponentsSchema(
  componentsList: string[],
  prefix: "islands" | "components",
  directory: string,
): Promise<SchemaMap[]> {
  ...
  return modules.map((componentModule, index) => {
    const componentNameRegex = /^\/(\w*)\.(tsx|jsx|js|ts)/;

    if (!componentNameRegex.test(componentsList[index])) {
      return null;
    }

    const componentName = componentsList[index].replace(
      componentNameRegex,
      "$1",
    );
...
export async function generate(directory: string, manifest: DevManifest) {
  ...
  const schemas = [
    ...(await extractComponentsSchema(components, "components", directory)),
    ...(await extractComponentsSchema(islands, "islands", directory)),
  ];
