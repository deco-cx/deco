...
export default {
  state: {
    invoke: function state(_props, { resolvables, resolvers }) {
      return {
        resolvables,
        resolvers,
      };
  ...
  resolvables: {
    invoke: function resolvables(_props, { resolvables }) {
      return resolvables;
    },
  },
  resolvers: {
    invoke: function resolvers(_props, { resolvers }) {
      return resolvers;
    },
  },
  once: {
    invoke: function once({ key, func }, { runOnce }) {
      return runOnce(key ?? func.name, func);
    },
  },
  resolveTypeSelector: {
    invoke: function resolveTypeSelector(
  ...
  mergeProps: {
    invoke: <T>({ props }: { props: Partial<T>[] }) => {
      let result = {};
  ...
  resolved: {
    invoke: <T, R extends { data: T; deferred?: boolean }>(
      props: R,
      ctx: BaseContext,
    ) => {
