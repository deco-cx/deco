// copied from: https://github.com/timreichen/importmap/blob/4cad65a5daee447d736c3515888d94e1a02d3d3b/mod.ts
// under MIT License.
import {
    type ImportMap,
    isImportMap,
...
function sortAndNormalizeSpecifierMap(
    ...
    for (const [specifierKey, value] of Object.entries(originalMap)) {
        ...
        if (
            ...
            !serializeURL(addressURL).endsWith("/")
        ) {
            console.warn(
...
export function resolveImportMap(
    ...
    if (imports !== undefined) {
        if (!isImports(imports)) {
            throw new TypeError(
                `"imports" top-level key needs to be an object.`,
            );
        }
    ...
    if (scopes !== undefined) {
        if (!isScopes(scopes)) {
            throw new TypeError(
                `"scopes" top-level key needs to be an object.`,
            );
        }
...
export function resolveModuleSpecifier(
    ...
    const normalizedSpecifier = asURL !== null
        ? serializeURL(asURL)
        : specifier;
