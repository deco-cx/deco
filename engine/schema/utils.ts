...
const extendsTypeFromNode = async (
  ...
  const newRoots = await denoDoc(fromFileUrl(rootNode.importDef.src));
  ...
  return extendsTypeFromNode(node, type);
};
      ...
      if (ast.kind === "variable") {
        ...
        if (
          ...
          (variableTsType.fnOrConstructor.tsType.repr === importAlias ||
            extendsType(variableTsType.fnOrConstructor.tsType, asts, typeName))
        ) {
          return {
            name: ast.name,
            params: variableTsType.fnOrConstructor.params.map(
