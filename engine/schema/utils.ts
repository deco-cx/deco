...
import { denoDoc as docAsExec } from "./denodoc_exec.ts";
import { denoDoc as docAsLib } from "./denodoc_wasm.ts";
...
export const denoDoc = (
  ...
  if (typeof Deno.run !== "function") {
    return docAsExec(path, importMap);
  }
  return docAsLib(path, importMap);
};
