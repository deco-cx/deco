...
const environments: UpgradeOption = {
  ...
  apply: async () => {
    ...
    const addNewTasks = async (): Promise<Patch> => {
      ...
      const start =
        `deno task bundle && deno run --unstable -A ${envArg} --unstable --config=deno.json $(deno eval 'console.log(import.meta.resolve("deco/hypervisor/main.ts"))') --build-cmd 'deno task build' -- deno task dev`;
