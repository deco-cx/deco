...
const runModule = (module: string, ...args: string[]): Deno.Command => {
    return new Deno.Command(Deno.execPath(), {
        args: ["run", "-A", "--import-map", join(Deno.cwd(), "import_map.json"), import.meta.resolve(module), ...args], stdout: "piped",
        stderr: "piped",
    })
}
...
while (true) {
    ...
    for await (const event of it) {
        if (event.paths.some(path => path.endsWith(".tsx") || path.endsWith(".ts"))) {
            break;
        }
    }
