        ...
        if (symbolMapForModule) {
            namedImports.forEach((namedImport) => {
                ...
                if (symbolMapForModule[name]) {
                    ...
                    const namedImports =
                        newImports[newModuleSpecifier].namedImports;
...
const applyPatch = async (p: FileMod): Promise<void> => {
    if (isDelete(p)) {
        await Deno.remove(p.from.path).catch(() => {});
    } else {
...
const applyCodeMod = async ({ patches, name, description }: CodeMod) => {
    ...
    for (const patch of patches) {
        ...
        const prettyPath = fromPath.replaceAll(Deno.cwd(), ".");
        ...
        console.log(
            `⚠️  ${brightYellow(prettyPath)} -> ${
                brightYellow(patch.to.path.replaceAll(Deno.cwd(), "."))
            }`,
        );
...
> {
    ...
    description?: string;
    context?: TContext;
    targets: CodeModTarget<TContext>[];
}
export const codeMod = async <TContext extends CodeModContext = CodeModContext>(
    { name, description, targets, context }: CodeModOptions<TContext>,
) => {
    const patches: FileMod[] = [];
    const ctx = context ?? {} as TContext;
    for (const target of targets) {
        for await (const file of walk(Deno.cwd(), target.options)) {
            if (file.isFile) {
                const from: TextFile = {
    ...
    await applyCodeMod({ name, description, patches });
};
